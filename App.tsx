...
export default function App() {
  ...
  useEffect(() => {
    const savedGeminiKey = localStorage.getItem('aura_gemini_key');
  ...
  const handleGenerate = async () => {
    ...
    if (currentModel === 'gemini-3-pro-image-preview' && !geminiApiKey) {
    ...
    if (internalMode === EditMode.VIEW_SHIFT) {
      ...
      if (engine === 'SEEDREAM') {
         finalPrompt = `${prefix}${viewText}${poseInstr}。${prompt ? `${prompt}。` : ""}关键：画面的透视关系需精确匹配视角需求，背景与主体光影融合，保持主体一致。`;
      } else {
    ...
    try {
      ...
      if (errorStr.includes('PERMISSION_DENIED') || errorStr.includes('403') || errorStr.includes('permission')) {
        setStatus({ 
          isLoading: false, 
          error: "Gemini 3 Pro 需要付费项目密钥。请在设置中输入有效的 API Key，或使用系统选择器。", 
          step: 'idle' 
        });
        if (!geminiApiKey) await (window as any).aistudio.openSelectKey();
      } else {
        setStatus({ 
          isLoading: false, 
          error: err.message || "生成失败，请检查密钥配置或网络连接。", 
          step: 'idle' 
        });
      }
    }
    ...
    <div className="min-h-screen pt-20 pb-10 flex flex-col items-center">
      ...
      <BottomSheet isOpen={showSettings} onClose={() => setShowSettings(false)} title="系统配置">
        <div className="space-y-6">
           {/* Gemini Configuration - Manual Input Version */}
           <div className="bg-[#F2F2F7] rounded-xl p-6 border border-black/5 space-y-4">
              ...
              <IOSInput 
                ...
                value={geminiApiKey} 
                onChange={(e) => handleGeminiKeyChange((e.target as any).value)} 
                type="password" 
              ...
              <div className="pt-2">
                <p className="text-[11px] text-[#86868B] leading-relaxed mb-3">
                  高质量渲染 (2K/4K) 需要绑定付费项目的密钥。如手动密钥无效，请尝试系统选择器。
                </p>
                <div className="flex flex-col gap-2">
                  <button 
                    ...
                    className="flex items-center justify-center gap-2 py-2.5 bg-white text-[#007AFF] border border-[#007AFF]/20 rounded-xl text-xs font-bold hover:bg-blue-50 transition-all"
                  >
                  ...
                  <a 
                    ...
                    className="flex items-center justify-center gap-1.5 text-[11px] text-[#86868B] hover:text-[#007AFF] transition-colors"
                  >
          ...
          <GlassCard className="flex-1 flex items-center justify-center relative overflow-hidden bg-white/50 border-dashed border-2 border-black/5 hover:border-black/10 transition-colors group">
            {status.isLoading && (
              <div className="absolute inset-0 bg-white/60 backdrop-blur-sm z-20 flex flex-col items-center justify-center gap-4">
                 ...
                 {isGemini3 && <p className="text-[10px] text-blue-500 px-10 text-center">Gemini 3 Pro 高质量渲染中，建议使用付费项目密钥</p>}
              </div>
            )}
            ...
            <div className="space-y-4 pt-2 border-t border-black/5">
              ...
              {isGemini3 && (
                <div className="mt-2 flex items-center gap-2 p-2 bg-blue-50 border border-blue-100 rounded-lg">
                   ...
                   <p className="text-[10px] text-blue-600">2K/4K 渲染建议使用已在设置中配置的有效 API Key</p>
                </div>
              )}
            </div>
          </GlassCard>
